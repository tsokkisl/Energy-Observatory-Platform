  ...
  connection.query("SELECT Id FROM resolutioncode WHERE ResolutionCodeText = '" + req.params.resolution + "'", function (err, result, fields) {
    ...
    "' AND a.Day = '" + date.getDate() + "' ORDER BY a.DateTime ASC", function (err, result, fields) {
      ...
      if (result.length == 0 || result == undefined) {
        ...
        for (var x in result) {
          data.push({ "Source" : "entso-e", "Dataset" : "ActualTotalLoad", "AreaName" : result[x].AreaName, "AreaTypeCode" : result[x].AreaTypeCodeText, "MapCode" : result[x].MapCodeText,
          ...
          "ActualTotalLoadValue" : result[x].TotalLoadValue, "UpdateTimeUTC" : result[x].UpdateTime });
        }
...
router.get('/:areaName/:resolution/month/:date/', isAuthenticated, resetQuota, checkForResources, (req, res) => {
  ...
  if (date.length != 2) res.sendStatus(400);
    ...
    connection.query("SELECT Id FROM resolutioncode WHERE ResolutionCodeText = '" + req.params.resolution + "'", function (err, result, fields) {
      ...
      connection.query("SELECT a.*, SUM(a.TotalLoadValue) as sumPerDay, b.AreaTypeCodeText, c.MapCodeText, d.ResolutionCodeText FROM actualtotalload a join areatypecode b on a.AreaTypeCodeId = b.Id join mapcode c on a.MapCodeId = c.Id join resolutioncode d on a.ResolutionCodeId = d.Id WHERE a.AreaName = '" + req.params.areaName + "' AND a.ResolutionCodeId = '" + result[0].Id + "' AND a.Year = '" + year + "' AND a.Month = '" + month + "' GROUP BY a.Day ORDER BY a.DateTime ASC", function (err, result, fields) {
        if (err) throw err;
        let data = [];
        if (result.length == 0 || result == undefined) {
          sendData(req, res, null);
          ...
          for (var x in result) {
            data.push({ "Source" : "entso-e", "Dataset" : "ActualTotalLoad", "AreaName" : result[x].AreaName, "AreaTypeCode" : result[x].AreaTypeCodeText, "MapCode" : result[x].MapCodeText,
            "ResolutionCode" : result[x].ResolutionCodeText, "Year" : result[x].Year, "Month" : result[x].Month, "Day" : result[x].Day, "ActualTotalLoadByDayValue" : result[x].sumPerDay });
          }
          sendData(req, res, data);
        }
        useAPIResource(JSON.parse(localStorage.getItem('authUser'))['username']);
...
router.get('/:areaName/:resolution/year/:date/', isAuthenticated, resetQuota, checkForResources, (req, res) => {
  ...
  if (date.length != 1) res.sendStatus(400);
    ...
    connection.query("SELECT Id FROM resolutioncode WHERE ResolutionCodeText = '" + req.params.resolution + "'", function (err, result, fields) {
      ...
      connection.query("SELECT a.*, SUM(a.TotalLoadValue) as sumPerMonth, b.AreaTypeCodeText, c.MapCodeText, d.ResolutionCodeText FROM actualtotalload a join areatypecode b on a.AreaTypeCodeId = b.Id join mapcode c on a.MapCodeId = c.Id join resolutioncode d on a.ResolutionCodeId = d.Id WHERE a.AreaName = '" + req.params.areaName + "' AND a.ResolutionCodeId = '" + result[0].Id + "' AND a.Year = '" + year + "' GROUP BY a.Month ORDER BY a.DateTime ASC", function (err, result, fields) {
        if (err) throw err;
        let data = [];
        if (result.length == 0 || result == undefined) {
          sendData(req, res, null);
          ...
          for (var x in result) {
            data.push({ "Source" : "entso-e", "Dataset" : "ActualTotalLoad", "AreaName" : result[x].AreaName, "AreaTypeCode" : result[x].AreaTypeCodeText, "MapCode" : result[x].MapCodeText,
            "ResolutionCode" : result[x].ResolutionCodeText, "Year" : result[x].Year, "Month" : result[x].Month, "ActualTotalLoadByMonthValue" : result[x].sumPerMonth });
          }
          sendData(req, res, data);
        }
        useAPIResource(JSON.parse(localStorage.getItem('authUser'))['username']);
